...
void controlHeaterOnOff() {
  ...
  if (currSettings.heaterOn && (heaterAlwaysOff || currSettings.aquaTempStatus != normal || currSettings.aquaTemp >= maxTemp)) {
    currSettings.heaterOn = false;
    Module.setLED(0, 4);
    digitalWrite(HEATER_PIN, HIGH);
  }
  else if (!currSettings.heaterOn && !heaterAlwaysOff && currSettings.aquaTempStatus == normal && currSettings.aquaTemp <= minTemp) {
  ...
  if (currSettings.now.minute == 0) {
    ...
    word _logValue;
    if (currSettings.aquaTempStatus != normal) _logValue = 0;
  ...
  switch (_countWaiting) {
    ...
    case 10:
      ...
      if (_lastTime == 0 || (_currentTime - _lastTime) > 60000) {
        if (currSettings.aquaTempStatus == notBegin) {
          if (sensor.begin()) currSettings.aquaTempStatus = readTemp;
        }
        if (currSettings.aquaTempStatus == readTemp) {
          sensor.requestTemperatures();
          _countWaiting = 1;
        }
        _lastTime = _currentTime;
