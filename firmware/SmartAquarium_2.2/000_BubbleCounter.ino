...
void BubbleCounter::endingBubbleInterval(word& __durationBubble, int& __levelBubble, int& __maxLevelBubble, byte& __bubblePart, byte& _events) {
  __durationBubble = 0;
  __levelBubble = 0;
  __maxLevelBubble = 0;
  __bubblePart = 0;
...
void BubbleCounter::tick() {
  ...
  static byte __bubblePart = 0; // индикатор части пузыря: 0 -нет, 1 - подъем был, 2 - спуск был
  ...
  if (__bubblePart == 0) {
    ...
    else if (__bubblePart == 1 && _intervalLevel < CHANGE_LEVEL_BUBBLE * (-1)) __bubblePart = 2;
    else if (__bubblePart == 2 && _intervalLevel >= CHANGE_LEVEL_BUBBLE * (-1) && _intervalLevel <= CHANGE_LEVEL_BUBBLE) {
