...
void endingBubbleInterval(word& __durationBubble, int& __levelBubble, int& __maxLevelBubble, byte& __bubblePart, byte& _events) {
  __durationBubble = 0;
  __levelBubble = 0;
  __maxLevelBubble = 0;
  __bubblePart = 0;
...
void debugCounterTick() {
  ...
  Serial.println("START of algoritm");
  ...
  while (_curIndex < BUFFER_SENSOR_SIZE) {
    ...
    if (__bubblePart == 0) {
      ...
      if (__durationBubble > _maxDurationBubble) {
        // Bubble too long
        endingBubbleInterval(__durationBubble, __levelBubble, __maxLevelBubble, __bubblePart, _events);
      }
      else if (__bubblePart == 1 && _intervalLevel < _changeLevelBubble * (-1)) __bubblePart = 2;
      else if (__bubblePart == 2 && _intervalLevel >= _changeLevelBubble * (-1) && _intervalLevel <= _changeLevelBubble) {
    ...
    Serial.print(",__bP: "); Serial.print(__bubblePart);
  ...
  Serial.println("END of algoritm: ");
}*/
